...
options="--color=0.611,0.811,0.847 --bordersize=3.0 --select --hidecursor --nodrag"

case $1 in
'clip')
        file=$(date +%s).png
        maim $options | tee /tmp/$file | xclip -selection clipboard -t image/png
        dunstify -r 69 -u low "Screenshot taken to clipboard"
        ;;
'save')
        filename=$(date +%s)
        path=~/Pictures/screenshots
        maim $options ~/Pictures/screenshots/$filename.png
